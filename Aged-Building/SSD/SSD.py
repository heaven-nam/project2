...
def process_dataset(data_dir):
    paths_filename = {}
    for root, dirs, files in tqdm(os.walk(data_dir), desc="Searching for files"): #  'E:/seoul/data/Training'
        for file in files:
    ...
    for img, label in tqdm(zip(images_paths, labels_paths), desc="Data processing"):
        ...
        f = open(os.path.join(data_dir, label), "r", encoding = 'utf8')
        ...
        if img_type == "R":
            ...
            if info['boxes']:
                images.append(img.strip(data_dir) + 'g')
                labels.append(info)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = cv2.imread(os.path.join(self.dir, 'ra' + self.images[index]))
...
class Model(Module):
    ...
    def build_model(self, num_classes, device, parallel):
        model = detection.ssd300_vgg16(
            trained=False, num_classes=num_classes, pretrained_backbone=False)
